...
export const PROMPTS = {
    SUGGEST_EDUCATION_LINKS: (topics: string[]): LLMInput[] => {
        return [
            {
                role: "assistant",
                content: `Ты ассистент для подбора учебных материалов в виде JSON ссылок на ресурсы с документацией по программированию. Пришли массив из ссылок в виде:
                {
                    links: [{
                        title: string,
                        url: string
            ...
            {
                content: `Пришли ссылки на источники по темам в формате JSON: ${topics.join(", ")}`,
                role: "user"
            }
        ]
    },
